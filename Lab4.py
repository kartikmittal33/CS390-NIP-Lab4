...
def buildGAN(images, discriminator_ratio, generator_ratio, epochs=40000, batchSize=32, loggingInterval=0):
    # Setup
    opt = Adam(lr=0.0002)
    ...
    discriminator_false_loss = []
    generator_loss = []
    for epoch in range(epochs):

        for r in range(discriminator_ratio):
            # Train discriminator with a true and false batch
            batch = images[np.random.randint(0, images.shape[0], batchSize)]
            batch = batch.reshape(batchSize, images.shape[1], images.shape[2], 1)
            ...
            discriminator_true_loss.append(advTrueLoss[0])
            discriminator_false_loss.append((advFalseLoss[1]))

        for r in range(generator_ratio):
            # Train generator by training GAN while keeping adversary component constant
            noise = np.random.normal(0, 1, (batchSize, NOISE_SIZE))
            genLoss = gan.train_on_batch(noise, trueCol)
            generator_loss.append(genLoss)
    ...
    plt.figure(1)
    plt.plot(range(len(discriminator_false_loss)), discriminator_false_loss, color='red', label='false')
    plt.plot(range(len(discriminator_true_loss)), discriminator_true_loss, color='blue', label='true')
    plt.legend()
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
...
def main():
    ...
    (generator, adv, gan) = buildGAN(data, discriminator_ratio, generator_ratio, epochs=20000, loggingInterval=1000)
